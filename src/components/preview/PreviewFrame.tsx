...
import {
  createImportMap,
  createPreviewHTML,
} from "@/lib/transform/tsx-transformer";

export function PreviewFrame() {
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const { files, refreshTrigger } = useFileSystem();
  const [error, setError] = useState<string | null>(null);
  const [entryPoint, setEntryPoint] = useState<string>("/App.tsx");
  ...
  useEffect(() => {
    ...
    updatePreview();
  }, [refreshTrigger, files, entryPoint, error, isFirstLoad]);
